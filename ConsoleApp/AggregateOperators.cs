...
public static class AggregateOperators {
    ...
    public static void SickLeaveHours() {
        ...
        int count = employees.Count(e => e.SickLeaveHours < 21);
    ...
    public static void MinProjection() {
        ...
        int minScore = scores.Min(s => s.First + s.Second);
    ...
    public static void MinBySyntax() {
        ...
        var playerWithMinScore = playersWithTotalScore.Min(p => p.Total);
    ...
    public static void MaxProjection() {
        ...
        int longestLength = categories.Max(c => c.Length);
    ...
    public static void SeededAggregate() {
        ...
        double endBalance = attemptedWithdrawals.Aggregate(startBalance, (endBalance, next) => (endBalance - next) > 0 ? endBalance -= next : endBalance); ;
    ...
    public static void NestedCount() {
        ...
        IEnumerable<(string Name, int RollCount)> diceCounts = players.Select(p => (p.Name, p.DiceRolls.Count));
